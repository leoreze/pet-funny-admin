...
app.get('/api/breeds', async (req, res) => {
  try {
    ...
    const rows = await db.all(
      `
      SELECT id, name, history, size, coat, characteristics, is_active, updated_at
      FROM dog_breeds
...
app.post('/api/breeds', async (req, res) => {
  try {
    const name = String(req.body.name || '').trim();
    const history = req.body.history != null ? String(req.body.history) : '';
    const size = req.body.size != null ? String(req.body.size) : '';
    const coat = req.body.coat != null ? String(req.body.coat) : '';
    const characteristics = req.body.characteristics != null ? String(req.body.characteristics) : '';
    const is_active = req.body.is_active === false ? false : true;
    ...
    const row = await db.get(
      ...
      VALUES ($1,$2,$3,$4,$5,$6,NOW())
      RETURNING id, name, history, size, coat, characteristics, is_active, updated_at
      `,
